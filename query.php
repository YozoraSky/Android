<?php
	...
	$num = $db->query("SELECT count(ORDERNO) FROM product where ORDERNO = '$orderno'"); //計算同單號的資料共有幾筆!
	$row = mysqli_fetch_array($num);

	if($stmt = $db->query("SELECT * FROM product where ORDERNO='$orderno' and ORDERSN = '$ordersn'"))//如果ordersn是數字的話，則不須加上單引號，但若包含了字母，則須加上單引號
	{
